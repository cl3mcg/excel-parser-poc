...
/**
 * Represents a column with a probability score.
 * @typedef { Object } ProbableColumn
 * @property {number} columnIndex - The index of the column in the dataset.
 * @property {string} columnName - The name of the column in the dataset.
 * @property {number} bestProbabilityResult - The probability score indicating the likelihood of the column containing lane ID data.
*/
...
* @returns {Array<ProbableColumn>} An array of ProbableColumn objects representing the columns most likely to contain lane ID data.
*
* @example
...
const returnTheMostProbableLaneIdColumn = function (uniqueColumnsNamesArray) {
  ...
  const fuzzySet_headerNames_laneId = FuzzySet(headerNames_laneId);
  ...
  */
  ...
  if (probabilityResult.filter(el => el.bestProbabilityResult >= 0.9).length >= 2) {
    ...
    console.log(`🎯 "Lane ID" column selected is: `, probabilityResult.sort((el1, el2) => el2.bestProbabilityResult - el1.bestProbabilityResult).map(el => el.columnName)[0]);
